...
class GigInvite extends Component<GigInviteProps, GigInviteState> {
  ...
  handleAccept = async ():Promise<void> => {
    ...
    this.context.authenticate(this.context.token ?? localStorage.getItem('token') ?? '')
  };
          ...
          <Grid container justifyContent="center" padding={4} spacing={0}>
            <Grid item xs={12} display='flex' justifyContent='center'>
              <Typography variant="h5">
                {this.props.details.bandLeader.name.split(' ')[0]} has sent you an offer!
              </Typography>
            </Grid>
            <div style={{height: 55}} />
            <Grid item xs={12} display='flex' justifyContent='center'>
              <Typography variant='subtitle2'>Do you want to play {this.state.role} on this gig?</Typography>
            </Grid>
            <div style={{height: 90}} />
            <Grid
              display="flex"
              ...
              xs={6}
            >
              <Button onClick={this.handleDecline} variant="contained" color="error">
                DECLINE
              </Button>
            </Grid>
            <Grid
              ...
              xs={6}
            >
              <Button onClick={this.handleAccept} variant="contained" color="success">
                ACCEPT
              </Button>
            </Grid>
          </Grid>
